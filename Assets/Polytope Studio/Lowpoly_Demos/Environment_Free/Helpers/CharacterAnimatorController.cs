...
public class CharacterAnimatorController : MonoBehaviour
{
    ...
    public void SetWalking(bool isWalking)
    {
        ...
        // 走路时关闭待机
        if (isWalking && !string.IsNullOrEmpty(idleParamName))
        {
    ...
    public void SetTalking(bool isTalking)
    {
        ...
        // 开始说话时，关闭走路（实现 Walk → Talk 平滑过渡）
        if (isTalking && !string.IsNullOrEmpty(walkParamName))
        {
    ...
    public void SetIdle(bool isIdle)
    {
        ...
        // 待机时关闭走路
        if (isIdle && !string.IsNullOrEmpty(walkParamName))
        {
            _animator.SetBool(walkParamName, false);
        }
    }
